...
impl BertAttention {
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        let key_transposed = key
            .transpose(key.dims().len() - 2, key.dims().len() - 1)
        ...
        let attention_probs =
            candle_nn::ops::softmax(&attention_scores, attention_scores.dims().len() - 1)
        ...
        let (batch_size, seq_length, _, _) = context
            .dims4()
...
impl MiniLMModel {
    pub fn new(model_id: &str) -> Result<Self> {
        ...
        for i in 0..config.num_hidden_layers {
            let layer = BertLayer::new(vb.clone(), &format!("encoder.layer.{}", i), &config)?;
    ...
    fn mean_pooling(&self, embeddings: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let attention_mask = attention_mask
            .unsqueeze(2)? // Add hidden dimension
    ...
    fn forward(&self, input_ids: &Tensor, position_ids: &Tensor) -> Result<Tensor> {
        ...
        let mut hidden_states = self
            .embed_tokens(input_ids, position_ids)
